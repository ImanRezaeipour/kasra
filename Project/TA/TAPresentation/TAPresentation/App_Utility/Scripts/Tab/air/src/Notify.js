...
Ext.air.Notify = Ext.extend(Ext.air.NativeWindow, {
	...
	xtpl: new Ext.XTemplate(
		'<html><head><link rel="stylesheet" href="{extAllCSS}" /></head>',
			'<body>',
				'<div dir="rtl" class="{boxCls}-tl"><div class="{boxCls}-tr"><div class="{boxCls}-tc"></div></div></div><div class="{boxCls}-ml"><div class="{boxCls}-mr"><div class="{boxCls}-mc">',
			    	'<div dir="rtl" id="{msgId}">',
			    		'<span>{msg}</span>',
						'<div dir="rtl" id="{iconId}" style="float: right;"><img src="{icon}"></div>',
			    	'</div>',
