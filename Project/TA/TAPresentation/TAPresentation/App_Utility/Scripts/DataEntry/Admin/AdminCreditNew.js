...
var XmlBatchSave = {
    ...
    Remove: function (rowIndex) {
        ...
        var XmlNode = $(xml).find("Root").find("Tb").filter(function () {
    ...
    Add: function (Row) {
        ...
        this.Context = this.Context.replace("<Root>", "").replace("</Root>", "").replace("<Root/>", "");
        this.Context = this.Context + "<Tb>"
        ...
        this.Context = "<Root>" + this.Context + "</Root>";
    },
    ...
    ISEmpty: function () {
        if (this.Context.replace("<Root>", "").replace("</Root>", "").replace("<Root/>", "").trim() == "")
            return true;
        return false;
    },
            ...
            if ($(this).attr("id") != "TRInsert") {
                if ($(this).find("td:eq(0)").find("input:text").length > 0) {
                    if (NotEmpty(this, 2)) {
                        ...
                        XmlBatchSave.Context = "<Root></Root>";
                        return false;
                    }
                }
                else {
...
function SetFocusItem(SelecteItem) {
    ...
    //لود اطلاعات تکمیلی 
    if (($(LastSelectedRow).attr("date").trim() != $("#TblExtended").attr("date").trim()) ||
