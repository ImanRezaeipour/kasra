...
(function ($, undefined) {
    ...
    $.extend(Datepicker.prototype, {
        ...
        _generateHTML: function (inst) {
			...
			'.datepicker._hideDatepicker();" style="cursor:pointer">' + this._get(inst, 'closeText') + '</button>' : '');
            var buttonPanel = (showButtonPanel) ? '<div class="ui-datepicker-buttonpane ui-widget-content">' + (isRTL ? controls : '') +
			(this._isInRange(inst, gotoDate) ? '<button type="button" class="ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all" onclick="DP_jQuery_' + dpuuid +
			'.datepicker._gotoToday(\'#' + inst.id + '\');"' +
			' style="cursor:pointer">' + currentText + '</button>' : '') + (isRTL ? '' : controls) + '</div>' : '';
            ...
            var HolidayFileName = this._get(inst, 'HolidayFileName');
            var xmlDoc;
            var xmlNodes;
            ...
            if (HolidayFileName != '') {
                ...
                xhttp.open("GET", HolidayFileName, false);
                xhttp.send();
            ...
            for (var row = 0; row < numMonths[0]; row++) {
                ...
                for (var col = 0; col < numMonths[1]; col++) {
                    ...
                    for (var dow = 0; dow < 7; dow++) { // days of the week
                        ...
                        thead += '<th' + ((dow + firstDay + 6) % 7 == 4 ? ' class="ui-datepicker-week-end-header"' : '') + '>' +
                    ...
                    for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
                        ...
                        for (var dow = 0; dow < 7; dow++) { // create date picker days
                            ...
                            var HolidayDescr = "";
                            if (drawYear == printDate.getFullYear() && drawMonth == printDate.getMonth()) {
                                xmlNodes = xmlDoc.documentElement.selectNodes("/BaseInfoEntity/GetHolidayForCalendar[FYear=" + printDate.getFullYear() + "][FMonth=" + (printDate.getMonth() + 1) + "][FDay=" + printDate.getDate() + "]");
                                if (xmlNodes.length == 1) {
                                    HasHoliday = true;
                                    HolidayDescr = xmlNodes.item(0).selectSingleNode("Descr").text;
                            ...
                            tbody += '<td class="' +
							((dow + firstDay + 6) % 7 == 4 ? ' ui-datepicker-week-end' : '') + // highlight weekends
                            ...
                            (HasHoliday ? ' style="color:red;" title="' + HolidayDescr + '"' : ' ') +
