...
Calendar.dayMouseOver = function(ev) {
    ...
    if (el.ttip) {
        ...
        //alert(el.ttip)
        //alert(el.calendar.tooltips.className)
...
Date.prototype.print = function(str, dateType, useLangNumbers) {
    ...
    if (dateIsSpecial(y, m, d)) {
        if (DaySpecialTitle(m, d) != '' && DaySpecialTitle(m, d) != undefined)
            str = str + '(' + DaySpecialTitle(m, d) + ')'
    }
    //-----------------------
    return str;
};
...
Calendar._TT["PREV_YEAR"] = "سال قبل(موس را نگهداريد)";
Calendar._TT["PREV_MONTH"] = "ماه قبل(موس را نگهداريد)";
Calendar._TT["GO_TODAY"] = "رفتن به امروز";
Calendar._TT["NEXT_MONTH"] = "ماه بعد(موس را نگهداريد)";
Calendar._TT["NEXT_YEAR"] = "سال بعد(موس را نگهداريد)";
Calendar._TT["SEL_DATE"] = "انتخاب تاریخ";
//Calendar._TT["DRAG_TO_MOVE"] = "Drag to move";
Calendar._TT["DRAG_TO_MOVE"] = "حرکت تقويم";
...
//---------------------------بهاره قدمی
function DaySpecialTitle(month, day) {
//alert(1)
    var mt = SPECIAL_DAYS_Title[month];
    var strTitle = '';
    var m = SPECIAL_DAYS[month];
    strTitle = '';
    if (m) {
