...
SamplePanel = Ext.extend(Ext.DataView, {
    ...
    tpl : new Ext.XTemplate(
        '<div dir="rtl" id="sample-ct">',
            '<tpl for=".">',
            '<div dir="rtl"><a name="{id}"></a><h2><div>{title}</div></h2>',
            '<dl>',
                '<tpl for="samples">',
                    '<dd ext:url="{url}"><img src="shared/screens/{icon}"/>',
                        '<div dir="rtl"><h4>{text}</h4><p>{desc}</p></div>',
                    '</dd>',
                '</tpl>',
            '<div dir="rtl" style="clear:left"></div></dl></div>',
            '</tpl>',
        '</div>'
    ),
			...
			callback : function() {
				...
				if(window.console && window.console.firebug && ! cp.get('hideFBWarning')){
					var tpl = new Ext.Template(
						'<div dir="rtl" style="border: 1px solid #FF0000; background-color:#FFAAAA; display:none; padding:15px; color:#000000;"><b>Warning: </b> Firebug is known to cause performance issues with Ext JS. <a href="#" id="hideWarning">[ Hide ]</a></div>'			   
					);
