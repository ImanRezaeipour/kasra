...
CompleteColumn = function(){
    ...
    Ext.apply(this, {
        width: 22,
        header: '<div dir="rtl" class="task-col-hd"></div>',
        fixed: true,
		menuDisabled: true,
        id: 'task-col',
        renderer: function(){
            return '<div dir="rtl" class="task-check"></div>';
        },
        init : function(xg){
            grid = xg;
...
ReminderColumn = function(){
    ...
    Ext.apply(this, {
        width: 26,
        header: '<div dir="rtl" class="reminder-col-hd"></div>',
        fixed: true,
        id: 'reminder-col',
		menuDisabled: true,
        dataIndex:'reminder',
        renderer: function(v){
			return '<div dir="rtl" class="reminder '+(v ? 'reminder-active' : '')+'"></div>';
        },
        init : function(xg){
            grid = xg;
            grid.on('render', function(){
