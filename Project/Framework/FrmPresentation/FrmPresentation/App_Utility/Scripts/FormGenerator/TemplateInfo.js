...
function OnClickBtnDelete() {
    if (LastSelectedRow != null) {
        if (confirm("آيا براي حذف مطمئن هستيد؟")) {
            document.getElementById(MasterObj + 'txtTemplateID').value = LastSelectedRow.getAttribute("TemplateID")
...
function OnClickBtnNew() {
    var strUrl = "Template.aspx?ToPersonId=" + $get(MasterObj + "txtOnLineUser").value
                                + "&SessionID=" + $get(MasterObj + "txtSessionID").value
    strUrl = encodeURI(strUrl)
    var retValue = window.showModalDialog(strUrl, '', 'dialogHeight: 770px;dialogWidth:1000px;center: Yes;help: no;status: no')
    if (retValue == 1)
        OnClickBtnFilter()
}
