<%@ Page Title="" Language="C#" MasterPageFile="~/App_Utility/MasterPage/MasterBule.master"
    AutoEventWireup="true" CodeBehind="CodeCondition.aspx.cs" Inherits="FrmPresentation.App_Utility.CodeCondition.CodeCondition1" %>
                ...
                <table>
                    <tr>
                        ...
                        <td>
                            ...
                            <td>
                                <asp:DropDownList ID="CmbCodeType" runat="server">
                                </asp:DropDownList>
                            </td>
                    </tr>
                </table>
...
//                alert(XmlNodeTmp.length)
        ...
        function SetChk() {
            ...
            function OnClickBtnSearch() {
                ...
                else if ($get(MasterObj + "txtCode").value == "" && $get(MasterObj + "CmbCodeType").value!='0') {
                    $get(MasterObj + "txtSearchflag").value = "0"
                    document.getElementById(MasterObj + "txtSubmit").value = "Filter"
                    document.getElementById(MasterObj + "BtnSubmit").click()
                }
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!تایید کردن!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        function CreateStr() {
            ...
            oXmlDoc.loadXML("<DataEntryEntity>" + str1 + "</DataEntryEntity>");
            XmlNode = oXmlDoc.documentElement.selectNodes("/DataEntryEntity/CodeCondition");

            if (document.getElementById(MasterObj + "txtHidden").value != "SelectCode") {
                for (var i = 0; i < XmlNode.length; i++) {
                    if (XmlNode.item(i).selectSingleNode('Chk').text == "1") {
                        code = XmlNode.item(i).selectSingleNode('CodeID').text
                        Action = XmlNode.item(i).selectSingleNode('operand').text
                        val = XmlNode.item(i).selectSingleNode('val').text
                        if (HTM != "") {
                            if (c < 2)
                        ...
                        if (c < 2)
                            HTM += code + Action + val
                        TXT += code + Action + val
                        strChk += XmlNode.item(i).selectSingleNode('CodeID').text + "_" + Action + "$" + val + "#1;"
                        c++;
                    }
                }
            ...
            if (document.getElementById(MasterObj + "txtHidden").value == "SelectCode") {
                ...
                for (var i = 0; i < XmlNode.length; i++) {
                    if (XmlNode.item(i).selectSingleNode('Chk').text == "1") {
                        ...
                        HTM += code;
                        TXT += code;
                        strChk += XmlNode.item(i).selectSingleNode('CodeID').text;
                        c++;
...
//        //================================================SetChkSelectCode====================================================
        ...
        //برای زمانی کار دکمه جستجو زده میشود تا بتواند به صفحات قبل یا بعد برود
        function CheckSearch() {
            if ($get(MasterObj + "txtSearchFlag").value == "1") {
