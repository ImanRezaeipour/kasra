...
function CheckCorrectDate()
{
	...
	if (event.srcElement.id == strUserControlID + "_txtDay")
		if (day > 0 && day < 30 )    
		{
			...
			if (month == 0 ||
				(month != 0 && month < 12) ||
					(month != 0 && month == 12 && (year == 0 || (year != 0 && (year + 1)%4 == 0)))) //سال کبيسه
			{	
		...
		if (month > 0 && month <= 12) 
			if ((day == 0) || (year == 0) ||
				(day < 30 ) || (day == 30 && month < 12) ||
					(day == 30 && month == 12 && (year + 1)%4 == 0) || //سال کبيسه
	...
	//________________________________________________________	
		...
		if ((year >= 1300 && year <= 1400) || (year < 100 && year >= 0))
			if ((day == 0) || (month == 0) || 
				(day > 0 && day < 30) ||
					(day == 30 && (month < 12 || (month == 12 && (year + 1)%4 == 0))) ||//سال کبيسه
						(day == 31 && (month > 0 && month < 7)))
			{	
			...
			{
				//alert('تارِيخ را اصلاح نماييد')
				//document.all.item(strUserControlID + "_txtYear").select();
				return false
			}	
		else
		{
			//alert('سال اشتباه وارد شده')
			//document.all.item(strUserControlID + "_txtYear").select();
			return false
		}
...
function onKeyDownCheckContentText()
{
						...
						(event.keyCode != 8)) //BackSpace
	{
		...
		if ((event.srcElement.id.search('_txtDay') != -1 || event.srcElement.id.search('_txtMonth') != -1) && 
			(event.srcElement.value.length == 2))	//در صورتيکه طول اطلاعات برابر 2 بود براي روز و ماه فوکوس خودکار تنظيم گردد
		{
			var retValue = CheckCorrectDate() 
			if (retValue) 
			{
