...
function vbmenu_close()
{
	if (vbmenu_currentactive)
	{
		for (key in vbmenu_activemenus)
		{
			...
			fetch_object("Archive_Menu").style.display = "none";
...
function vbmenu_init(controlid)
{
	...
	if (document.getElementsByTagName && menuobj)
	{
		...
		controlobj.oncontextmenu = vbmenu_eventhandler_click;
		...
		popupobj.oncontextmenu = e_by_gum;
		...
		{			
			if (tds[i].className == "vbmenu_option")
			{
				...
				else
				{
					...
					tds[i].oncontextmenu = vbmenu_navtolink;
					...
					if (!is_saf && !is_kon)
					{
						try
						{
							...
							{
								if (typeof(links[j].oncontextmenu) == "undefined")
								{
									links[j].oncontextmenu = e_by_gum;
								}
							}
						}
						catch(e)
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 12:55, Wed Jul 7th 2004
